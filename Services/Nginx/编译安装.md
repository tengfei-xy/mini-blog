...
sudo chown -R nginx.nginx /usr/local/services/nginx-1.21.5
```
...
[Service]
Type=forking
ExecStartPre=/usr/local/services/nginx-1.21.5/sbin/nginx -t
ExecStart=/usr/local/services/nginx-1.21.5/sbin/nginx -c /usr/local/services/nginx-1.21.5/conf/nginx.conf
