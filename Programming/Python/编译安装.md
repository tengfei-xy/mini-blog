...
[编译选项参考](https://docs.python.org/zh-cn/3.10/using/configure.html)
...
./configure --prefix=/usr/local/Python3.6/ --enable-optimizations --enable-loadable-sqlite-extensions --with-ssl --with-system-ffi=/usr --enable-shared
#--prefix :指定安装目录
...
./configure --prefix=/usr/local/services/python3.8.9/  --enable-loadable-sqlite-extensions --enable-shared --with-system-ffi=/usr --enable-optimizations
...
./configure --prefix=/usr/local/services/python3.9.9/  --enable-loadable-sqlite-extensions --enable-shared --enable-optimizations --with-system-ffi=/usr 
make -j `grep processor /proc/cpuinfo  | wc -l` && sudo make install
sudo cp libpython3.* /usr/lib64/
```
